...
#define UDMA_HYPERBUS_PER_ID 5
#define UDMA_HYPERBUS_OFFSET 0x1a102000+128*UDMA_HYPERBUS_PER_ID
#define UDMA_HYPERBUS_CONFIG_REG_OFFSET 0x80
...
static inline void set_memsel_hyper(unsigned int mem_sel){
  pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x20, mem_sel); // hyperram 0, hyperflash 1, psram(x8) 2, psram(x16)3
}

static inline int check_memsel_hyper(){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x20);
}

static inline void set_en_latency_add(unsigned int en){
  pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x08, en ); // REG_T_EN_LATENCY_ADD enable 1, deactivated 0
}

static inline void set_t_latency_access(unsigned int latency){
  pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x04, latency);
}

static inline void set_t_cs_max(unsigned int period){
  pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x0C, period );
}
...
static inline void set_pagebound_hyper(unsigned int page_bound){
  ...
  switch(page_bound){
     case 128:
        pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, 0x00 ); // page boundary is set to every 128 bytes
        break;
     case 256:
        pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, 0x01 ); // page boundary is set to every 256 bytes
        break;
     case 512:
        pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, 0x02 ); // page boundary is set to every 128 bytes
        break;
     case 1024:
        pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, 0x03 ); // page boundary is set to every 256 bytes
        break;
     default:
        pulp_write32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, 0x04 ); // page boundary is not considered
  }
...
static inline void set_twd_param_hyper(unsigned int twd_act_ext, unsigned int twd_count_ext,unsigned int twd_stride_ext, unsigned int twd_act_l2, unsigned int twd_count_l2, unsigned int twd_stride_l2, unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x28, twd_act_ext    ); // 2D TRAN is deactivated
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x2C, twd_count_ext  ); // 2D COUNT for ext mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x30, twd_stride_ext ); // 2D STRIDE for ext mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x34, twd_act_l2     ); // 2D TRAN is deactivated
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x38, twd_count_ext  ); // 2D COUNT for l2
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x3C, twd_stride_ext ); // 2D STRIDE for l2
}

static inline void set_tx_param_hyper(unsigned int len, unsigned int ext_addr, unsigned int l2_addr, unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x0C, l2_addr ); // Data address in L2
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x10, len );     // Data size to be sent
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x1C, ext_addr );// external memory address
}

static inline void set_rx_param_hyper(unsigned int len, unsigned int ext_addr, unsigned int l2_addr, unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x00, l2_addr ); // Data address in L2
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x04, len );     // Data size
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x1C, ext_addr );// external memory address
}

static inline void set_hyper_cfg(unsigned int data, unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x20, data);
}

static inline void kick_reg_write_hyper(unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x18, 0x02); // Write is declared for the external mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x14); // Write transaction is kicked
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x00); // Write transaction information is reset
}

static inline void kick_mem_write_hyper(unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x18, 0x01); // Write is declared for the external mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x14); // Write transaction is kicked
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x00); // Write transaction information is reset
}

static inline void kick_flash_write_hyper(unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x18, 0x00); // Write is declared for the external mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x14); // Write transaction is kicked
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x14, 0x00); // Write transaction information is reset
}

static inline void kick_read_hyper(unsigned int tran_id){
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x18, 0x05);     // Read is declared for the external mem
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x08, 0x14);     // Read transaction is kicked
  pulp_write32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x08, 0x00);     // Read transaction information is reset
}


static inline void udma_hyper_setup(){
  pulp_write32(0x1a102000, 1 << UDMA_HYPERBUS_PER_ID); // clock for the hyper bus module is activated
...
static inline void udma_hyper_sleep(){
  ...
  pulp_write32(0x1a102000, (1 << UDMA_HYPERBUS_PER_ID)^a ); // Clock gating is activated
}
...
static inline int udma_hyper_nb_tran(unsigned int tran_id){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x24) >> 1;
}
...
static inline int udma_hyper_busy(unsigned int tran_id){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + tran_id*UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x24) & 0x00000001;
}
...
static inline int udma_hyper_id_alloc(){
  return pulp_read32(UDMA_HYPERBUS_OFFSET + UDMA_HYPERBUS_CONFIG_REG_OFFSET + 0x24);
}
